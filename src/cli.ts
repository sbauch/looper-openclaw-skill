...
interface HoleInfo {
  ...
  directionToHole?: number;
  par?: number;
  holeNumber?: number;
  strokeNumber?: number;
  hazards?: { type: string; distance: number; direction: number; description: string }[];
  availableClubs?: { name: string; expectedCarry: number; expectedTotal: number; recommendedPower?: number }[];
  stockYardages?: StockYardage[];
  asciiMap?: string | null;
  asciiLegend?: string | null;
  asciiAnalysis?: {
    linesOfPlay?: Array<{ name: string; club: string; target: string; distance: number; risk: string; notes?: string }>;
    hazards?: Array<{ type: string; location: string }>;
...
interface RoundState {
  ...
  parForHoles: Record<number, number>;
  teeColor: string;
}
...
interface AgentState {
  ...
  /** Agent display name */
  name?: string;
  /** Persisted server URL (saved on registration / explicit --serverUrl) */
  serverUrl?: string;
  /** Active round ID (set by `start`, used by `look`/`hit`/`view`/`scorecard`) */
  roundId?: string;
  /** Active course ID */
  courseId?: string;
  /** Course name for display */
  courseName?: string;
  /** Persisted map resolution preference (yards per ASCII cell, 2-20) */
  yardsPerCell?: number;
  /** Persisted tee color preference */
  teeColor?: string;
  /** Persisted map format preference */
  mapFormat?: 'ascii' | 'grid';
}
...
// ─── Stock Yardage type (comes from API, reflects agent's current skill) ─────

interface StockYardage {
  key: string;
  name: string;
  carry: number;
  total: number;
...
function getOption(options: Record<string, string | boolean>, key: string, fallback?: string) {
...
function getNumberOption(options: Record<string, string | boolean>, key: string, fallback: number) {
...
function resolveStatePath(options: Record<string, string | boolean>) {
...
async function readAgentState(statePath: string): Promise<AgentState | null> {
  try {
    ...
    if (!parsed.agentId || !parsed.apiKey) return null;
    return parsed;
  } catch {
    return null;
  }
...
async function writeAgentState(statePath: string, state: AgentState) {
...
class GolfApiClient {
  ...
  async getToken() {
    ...
    const data = await requestJson<{ token: string; expiresAt: number }>(
      `${this.serverUrl}/api/auth/agent-token`,
      {
        method: 'POST',
  ...
  private async authorizedFetch<T>(url: string, init?: RequestInit) {
    const token = await this.getToken();
    const headers = new Headers(init?.headers);
    headers.set('Authorization', `Bearer ${token}`);
    return requestJson<T>(url, { ...init, headers });
  }

  async listCourses() {
    return this.authorizedFetch<{ courses: CourseSummary[] }>(`${this.serverUrl}/api/courses`);
  }

  async startRound(courseId: string, teeColor: string, yardsPerCell?: number) {
    return this.authorizedFetch<{ round: RoundState; holeInfo: HoleInfo }>(
      `${this.serverUrl}/api/course/${courseId}/rounds`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async resumeRound(courseId: string, roundId: string) {
    return this.authorizedFetch<{ round: RoundState }>(
      `${this.serverUrl}/api/course/${courseId}/rounds/${roundId}/resume`,
      { method: 'POST' }
    );
  }

  async getHoleInfo(courseId: string, roundId: string, yardsPerCell?: number, mapFormat?: string) {
    const params = new URLSearchParams();
    if (yardsPerCell) params.set('yardsPerCell', String(yardsPerCell));
    if (mapFormat) params.set('mapFormat', mapFormat);
    const qs = params.toString();
    const url = `${this.serverUrl}/api/course/${courseId}/rounds/${roundId}/hole-info${qs ? `?${qs}` : ''}`;
    return this.authorizedFetch<HoleInfo>(url);
  }

  async submitShot(courseId: string, roundId: string, decision: { club: string; aimDirection: number; power: number }) {
    return this.authorizedFetch<{
      ...
      `${this.serverUrl}/api/course/${courseId}/rounds/${roundId}/shot`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  async getHoleImage(courseId: string, roundId: string) {
    return this.authorizedFetch<{ imageUrl: string }>(
      `${this.serverUrl}/api/course/${courseId}/rounds/${roundId}/hole-image`
    );
  }
}

async function registerAgent(serverUrl: string, registrationKey: string, name?: string) {
  return requestJson<AgentState>(`${serverUrl}/api/agents/register`, {
    method: 'POST',
    headers: {
...
function normalizeClubName(name: string) {
  ...
  if (!trimmed) return '';
  if (trimmed === 'pw' || trimmed === 'sw') return trimmed;
  if (trimmed === 'putter' || trimmed === 'driver') return trimmed;
  if (trimmed.endsWith('-wood') || trimmed.endsWith('-iron')) return trimmed;
  return trimmed.replace(/\s+/g, '-');
}

function printHoleContext(holeInfo: HoleInfo) {
  console.log('');
  ...
  // Situation summary
  const parts: string[] = [];
  if (holeInfo.holeNumber != null) parts.push(`Hole ${holeInfo.holeNumber}`);
  if (holeInfo.par != null) parts.push(`Par ${holeInfo.par}`);
  if (holeInfo.strokeNumber != null) parts.push(`Stroke ${holeInfo.strokeNumber}`);
  if (holeInfo.ballLie) parts.push(`Lie: ${holeInfo.ballLie}`);
  if (holeInfo.distanceToHole != null) parts.push(`${holeInfo.distanceToHole.toFixed(0)}y to flag`);
  // directionToHole is only shown if the server includes it (controlled by config)
  if (holeInfo.directionToHole != null) parts.push(`Bearing: ${holeInfo.directionToHole.toFixed(0)} deg`);
...
function printShotResult(result: ShotResult) {
  console.log('');
  console.log(
    `Result: carry ${result.carry.toFixed(1)}y, roll ${result.roll.toFixed(1)}y, total ${result.totalDistance.toFixed(1)}y`
  );
...
function printScorecard(round: RoundState) {
  ...
  const scoreName = (strokes: number, par: number) => {
    const d = strokes - par;
...
async function cmdCourses(api: GolfApiClient) {
...
async function cmdStart(
  ...
  try {
    ...
    if (err.status === 409 && err.data && typeof err.data === 'object' && 'roundId' in err.data) {
      const roundId = String((err.data as { roundId?: string }).roundId || '');
      if (!roundId) throw err;
...
async function cmdLook(api: GolfApiClient, agentState: AgentState) {
...
async function cmdHit(
  ...
  if (!clubRaw) throw new Error('Missing --club. Example: --club driver');
  if (!aimRaw) throw new Error('Missing --aim (degrees 0-360). Example: --aim 355');
  if (!powerRaw) throw new Error('Missing --power (1-100). Example: --power 90');
...
async function cmdView(api: GolfApiClient, agentState: AgentState) {
  if (!agentState.courseId || !agentState.roundId) {
    throw new Error('No active round. Run `start` first.');
  }
...
async function cmdScorecard(api: GolfApiClient, agentState: AgentState) {
  if (!agentState.courseId || !agentState.roundId) {
    throw new Error('No active round. Run `start` first.');
  }
...
function cmdBearing(options: Record<string, string | boolean>) {
  ...
  if (Number.isNaN(ahead)) throw new Error('--ahead must be a number (yards toward the green, negative = behind)');
  if (Number.isNaN(right)) throw new Error('--right must be a number (yards right of ball, negative = left)');
